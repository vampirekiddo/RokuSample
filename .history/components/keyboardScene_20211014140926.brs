...
sub handleVideoPlaybackResponse()
    ...
    m.video.getChild(2).observeField("play", "handleNavigationEvents")
    m.video.getChild(2).observeField("forward", "handleNavigationEvents")
    m.video.getChild(2).observeField("back", "handleNavigationEvents")
    m.video.getChild(2).observeField("cc", "handleNavigationEvents")
end sub

sub updatePercentage()
    m.video.bufferingBar.height = 8
    ? m.video.bufferingBar.percentage.toStr() + "%"
    ?m.video.position
end sub

sub handleNavigationEvents()
