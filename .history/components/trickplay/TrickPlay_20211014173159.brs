...
sub updateLoading()
    ...
    m.loadingPercent.text = m.top.loadintgData.percentage.toStr() + "%"
    m.loadingFilled.width = INT(m.top.loadintgData.percentage * 1.5)
    if m.top.loadintgData.percentage = 100
        m.LoadingGroup.visible = false
    end if
end sub
